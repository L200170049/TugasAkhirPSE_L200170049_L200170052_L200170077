<?php
  ...
  if(isset($_POST['Simpan'])){
        if($aksi=='edit'){
            $sql_query = mysqli_query($conn, "UPDATE beli_kredit SET pembeli_no_ktp='".$_POST['pembeli_no_ktp']."', paket_kode='".$_POST['paket_kode']."', motor_kode='".$_POST['motor_kode']."', kredit_tanggal='".$_POST['kredit_tanggal']."', fotokopi_ktp='".$_POST['fotokopi_ktp']."', fotokopi_kk='".$_POST['fotokopi_kk']."', fotokopi_slip_gaji='".$_POST['fotokopi_slip_gaji']."' WHERE kredit_kode='".$_POST['kredit_kode']."'");
         }else{
             $sql_query = mysqli_query($conn, "INSERT INTO beli_kredit VALUES ('".$_POST['kredit_kode']."','".$_POST['pembeli_no_ktp']."','".$_POST['paket_kode']."','".$_POST['motor_kode']."','".$_POST['kredit_tanggal']."','".$_POST['fotokopi_ktp']."','".$_POST['fotokopi_kk']."','".$_POST['fotokopi_slip_gaji']."')") or die(mysql_error());
          }
  }

  if($aksi=='hapus' && isset($id)){
     $sql_query   = mysqli_query($conn, "DELETE FROM beli_kredit WHERE kredit_kode='".$id."'");
  }

  if($aksi=='edit' && isset($id)){
      $sql_display    = mysqli_query($conn, "SELECT *FROM beli_kredit WHERE kredit_kode='".$id."'");
...
<table width="900" cellspacing="0" id='input' border="0" align='center' cellpadding="0">
  ...
  <tr>
    ...
    <td>
      ...
      <?php 
          $sql_list = mysqli_query($conn, "SELECT *FROM kredit_paket");
          while($row_list=mysqli_fetch_assoc($sql_list)){
