<?php
  ...
  if(isset($_POST['Simpan'])){
        if($aksi=='edit'){
            $sql_query = mysqli_query($conn, "UPDATE kredit_paket SET paket_harga_cash='".$_POST['paket_harga_cash']."', paket_uang_muka='".$_POST['paket_uang_muka']."', paket_jumlah_cicilan='".$_POST['paket_jumlah_cicilan']."', paket_bunga='".$_POST['paket_bunga']."', paket_nilai_cicilan='".$_POST['paket_nilai_cicilan']."' WHERE paket_kode='".$_POST['paket_kode']."'");
         }else{
             $sql_query = mysqli_query($conn, "INSERT INTO kredit_paket VALUES ('".$_POST['paket_kode']."','".$_POST['paket_harga_cash']."','".$_POST['paket_uang_muka']."','".$_POST['paket_jumlah_cicilan']."','".$_POST['paket_bunga']."','".$_POST['paket_nilai_cicilan']."')") or die(mysqli_error($conn));      
         }
  }

  if($aksi=='hapus' && isset($id)){
     $sql_query   = mysqli_query($conn, "DELETE FROM kredit_paket WHERE paket_kode='".$id."'");
  }

  if($aksi=='edit' && isset($id)){
      $sql_display    = mysqli_query($conn, "SELECT *FROM kredit_paket WHERE paket_kode='".$id."'");
