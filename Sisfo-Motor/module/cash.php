...
$conn = mysqli_connect($servername, $uname, $pass, $database);
  ...
  if(!empty($page)){
	...
	$sql_display 	=mysqli_query($conn, "SELECT *FROM beli_cash INNER JOIN pembeli ON beli_cash.pembeli_no_ktp=pembeli.pembeli_no_ktp LIMIT $mulaiawal,$batas");
?>
...
<div class='isi'>
	...
	<br>
        ...
        <?php while($row=mysqli_fetch_assoc($sql_display)){ echo "
          <tr align='center'>
            <td>".$row['cash_kode']."</td>
...
<?php 
    $sqljumlahdata     = mysqli_query($conn, "SELECT *FROM beli_cash");  
    $jumlahdata        = mysqli_num_rows($sqljumlahdata);
