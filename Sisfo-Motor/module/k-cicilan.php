<?php
  ...
  if(isset($_POST['kredit_kode'])){
      $sql_change     = mysqli_query($conn, "SELECT *FROM beli_kredit INNER JOIN kredit_paket ON (beli_kredit.paket_kode=kredit_paket.paket_kode) WHERE kredit_kode='".$_POST['kredit_kode']."'");
      $row_display    = mysqli_fetch_assoc($sql_change);
      $cicilan_ke     = mysqli_query($conn, "SELECT * FROM bayar_cicilan WHERE kredit_kode='".$_POST['kredit_kode']."'");
      $cicilan_ke     = mysqli_num_rows($cicilan_ke);
  ...
  if(isset($_POST['Simpan'])){
        if($aksi=='edit'){
            $sql_query = mysqli_query($conn, "UPDATE bayar_cicilan SET kredit_kode='".$_POST['kredit_kode']."', cicilan_tanggal='".$_POST['cicilan_tanggal']."', cicilan_jumlah='".$_POST['cicilan_jumlah']."', cicilan_ke='".$_POST['cicilan_ke']."', cicilan_sisa_ke='".$_POST['cicilan_sisa_ke']."', cicilan_sisa_harga='".$_POST['cicilan_sisa_harga']."' WHERE cicilan_kode='".$_POST['cicilan_kode']."'");
         }else{
             $sql_query = mysqli_query($conn, "INSERT INTO bayar_cicilan VALUES ('".$_POST['cicilan_kode']."','".$_POST['kredit_kode']."','".$_POST['cicilan_tanggal']."','".$_POST['cicilan_jumlah']."','".$_POST['cicilan_ke']."','".$_POST['cicilan_sisa_ke']."','".$_POST['cicilan_sisa_harga']."')") or die(mysqli_error($conn));      
         }
  }

  if($aksi=='hapus' && isset($id)){
     $sql_query   = mysqli_query($conn, "DELETE FROM bayar_cicilan WHERE cicilan_kode='".$id."'");
  }
/*
  if($aksi=='edit' && isset($id)){
      $sql_display    = mysqli_query($conn, "SELECT *FROM bayar_cicilan WHERE cicilan_kode='".$id."'");
      $row_display    = mysqli_fetch_assoc($sql_display);
...
<table width="900" cellspacing="0" id='input' border="0" align='center' cellpadding="0">
  ...
  <tr>
    ...
    <td>
      ...
      <?php 
          $sql_list = mysqli_query($conn, "SELECT *FROM beli_kredit");
