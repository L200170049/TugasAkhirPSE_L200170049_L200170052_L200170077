...
if (isset($_POST['Simpan'])) {
    if ($aksi == 'edit') {
        $sql_query = mysqli_query($conn, "UPDATE beli_cash SET pembeli_no_ktp='" . $_POST['pembeli_no_ktp'] . "', motor_kode='" . $_POST['motor_kode'] . "', cash_tanggal='" . $_POST['cash_tanggal'] . "', cash_bayar='" . $_POST['cash_bayar'] . "' WHERE cash_kode='" . $_POST['cash_kode'] . "'");
    } else {
        $sql_query = mysqli_query($conn, "INSERT INTO beli_cash VALUES ('" . $_POST['cash_kode'] . "','" . $_POST['pembeli_no_ktp'] . "','" . $_POST['motor_kode'] . "','" . $_POST['cash_tanggal'] . "','" . $_POST['cash_bayar'] . "')") or die(mysql_error());
    }
}

if ($aksi == 'hapus' && isset($id)) {
    $sql_query = mysqli_query($conn, "DELETE FROM beli_cash WHERE cash_kode='" . $id . "'");
}

if ($aksi == 'edit' && isset($id)) {
    $sql_display = mysqli_query($conn, "SELECT *FROM beli_cash WHERE cash_kode='" . $id . "'");
...
<?php
$sql_list = mysqli_query($conn, "SELECT *FROM pembeli");
...
<?php
$sql_list = mysqli_query($conn, "SELECT *FROM motor");
while ($row_list = mysqli_fetch_assoc($sql_list)) {
