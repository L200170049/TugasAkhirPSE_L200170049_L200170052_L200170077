...
$conn = mysqli_connect($servername, $uname, $pass, $database);
  ...
  $cari   = $_POST['cari'];
	...
	$sql_display_1 	=mysqli_query($conn, "SELECT *FROM beli_kredit WHERE kredit_kode  LIKE '%".$cari."%'");
  $hitung       = mysqli_num_rows($sql_display_1);
  ...
  $sql_display_2  =mysqli_query($conn, "SELECT *FROM motor WHERE motor_kode  LIKE '%".$cari."%'");  
  $hitung       = mysqli_num_rows($sql_display_2);
    if($hitung>=1 && empty($kategori)){
      $kategori   = 'motor';
        $sql        = $sql_display_2; 
    }

  $sql_display_3  =mysqli_query($conn, "SELECT *FROM kredit_paket WHERE paket_kode LIKE '%".$cari."%'");
  $hitung       = mysqli_num_rows($sql_display_3);
    if($hitung >=1 && empty($kategori)){
      $kategori   = 'paket';
      $row  = mysqli_fetch_assoc($sql_display);
            $sql        = $sql_display_3;
    }

  $sql_display_4  =mysqli_query($conn,"SELECT *FROM pembeli  WHERE pembeli_no_ktp LIKE '%".$cari."%'");
  $hitung       = mysqli_num_rows($sql_display_4);
    if($hitung >=1 && empty($kategori)){
      $kategori   = 'pembeli';
            $sql        = $sql_display_4;
  ...
  $sql_display_5  =mysqli_query($conn, "SELECT *FROM beli_cash  WHERE cash_kode LIKE '%".$cari."%'");
  $hitung       = mysqli_num_rows($sql_display_5);
    if($hitung >=1 && empty($kategori)){
      $kategori   = 'cash';
            $sql        = $sql_display_5;
  ...
  $sql_display_6  =mysqli_query($conn, "SELECT *FROM bayar_cicilan  WHERE cicilan_kode LIKE '%".$cari."%'");
  $hitung       = mysqli_num_rows($sql_display_6);
    if($hitung >=1 && empty($kategori)){
      $kategori   = 'cicilan';
            $sql        = $sql_display_6;
...
?>
                ...
                <?php while($row=mysqli_fetch_assoc($sql)){ echo "
                  <tr align='center'>
