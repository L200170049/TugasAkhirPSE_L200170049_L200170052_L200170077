<?php
  ...
  if(isset($_POST['Simpan'])){
        if($aksi=='edit'){
            ...
            $sql_query = mysqli_query($conn, "UPDATE user SET username='".htmlentities(trim($_POST['username']))."', password='".htmlentities(trim(md5($password)))."'WHERE id_user='".$_POST['id_user']."'");
         }else{
             $sql_query = mysqli_query($conn, "INSERT INTO user (username,password) VALUES ('".htmlentities(trim($_POST['username']))."','".htmlentities(trim(md5($_POST['password'])))."')") or die(mysqli_error($conn));      
         }
  }

  if($aksi=='hapus' && isset($id)){
     $sql_query   = mysqli_query($conn, "DELETE FROM user WHERE id_user='".$id."'");
  }

  if($aksi=='edit' && isset($id)){
      $sql_display    = mysqli_query($conn, "SELECT *FROM user WHERE id_user='".$id."'");
