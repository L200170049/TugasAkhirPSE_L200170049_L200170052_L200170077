...
$conn = mysqli_connect($servername, $uname, $pass, $database);
  ...
  if(isset($_POST['Simpan'])){
        if($aksi=='edit'){
            $sql_query = mysqli_query($conn, "UPDATE pembeli SET pembeli_nama='".$_POST['pembeli_nama']."', pembeli_alamat='".$_POST['pembeli_alamat']."', pembeli_telpon='".$_POST['pembeli_telpon']."', pembeli_hp='".$_POST['pembeli_hp']."' WHERE pembeli_no_ktp='".$_POST['pembeli_no_ktp']."'");
         }else{
             $sql_query = mysqli_query($conn, "INSERT INTO pembeli VALUES ('".$_POST['pembeli_no_ktp']."','".$_POST['pembeli_nama']."','".$_POST['pembeli_alamat']."','".$_POST['pembeli_telpon']."','".$_POST['pembeli_hp']."')") or die(mysqli_error($conn));      
         }
  }

  if($aksi=='hapus' && isset($id)){
     $sql_query   = mysqli_query($conn, "DELETE FROM pembeli WHERE pembeli_no_ktp='".$id."'");
  }

  if($aksi=='edit' && isset($id)){
      $sql_display    = mysqli_query($conn, "SELECT *FROM pembeli WHERE pembeli_no_ktp='".$id."'");
