  ...
  if(isset($_POST['Simpan'])){
    if(!empty($_FILES['motor_gambar']['name'])){
        ...
        if($aksi=='edit'){
            ...
            $sql_query = mysqli_query($conn, "UPDATE motor SET motor_merk='".$_POST['motor_merk']."', motor_type='".$_POST['motor_type']."', motor_warna_pilihan='".$_POST['motor_warna_pilihan']."', motor_harga='".$_POST['motor_harga']."', motor_gambar='".$motor_gambar."' WHERE motor_kode='".$_POST['motor_kode']."'") or die(mysql_error());
         }else{
             $sql_query = mysqli_query($conn, "INSERT INTO motor VALUES ('".$_POST['motor_kode']."','".$_POST['motor_merk']."','".$_POST['motor_type']."','".$_POST['motor_warna_pilihan']."','".$_POST['motor_harga']."','".$motor_gambar."')") or die(mysqli_error($conn));      
         }
  }

  if($aksi=='hapus' && isset($id)){
     $sql_query   = mysqli_query($conn, "DELETE FROM motor WHERE motor_kode='".$id."'");
  }

  if($aksi=='edit' && isset($id)){
      $sql_display    = mysqli_query($conn, "SELECT *FROM motor WHERE motor_kode='".$id."'");
